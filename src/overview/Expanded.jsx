...
const Expanded = ({galleryThumbnails, revert, galleryList, mainImage, setMainImage}) => {
  ...
  let expandedCarousel = galleryList.map(image => {
    ...
    let imageStyle = {backgroundImage: `url(${url})`, backgroundSize: '3750px', backgroundPosition: position}
    return (
      <div id={imageID} className={ index === mainImage ? 'expanded-card' : 'inactive-card'}>
          ...
          {index === mainImage && (zoom ? <div className='zoom-window cursor-zoom-out' style={imageStyle} onMouseMove={handleMouseEvent}><img onClick={handleZoomClick} src={url}></img></div> : <img className='cursor-zoom-in' onClick={handleZoomClick} src={url}></img>)}
      </div>)
  })
